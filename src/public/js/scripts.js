(function($) {
    ...
    //assign item quantity before adding to cart and submit form
    $("#add_to_cart_btn").on('click', function() {
        ...
        //$('#add_item_to_cart').submit();
        $("#quantity").val($('#set_quantity').val());

        $('#add_to_cart_btn').html('Adding <span class="dot one">&#8226;</span><span class="dot two">&#8226;</span><span class="dot three">&#8226;</span>');
        ...
        setTimeout(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.cart-box h5').html(response.title);
                    $('.cart-box p').html(response.msg);
                    $('#cart_counter_div').removeClass('d-none');
                    $('#cart_circle').removeClass('d-none');
    ...
    $("#add_to_wishlist_btn").on('click', function() {
        ...
        $('#add_to_wishlist_btn').html('Adding <span class="dot one">&#8226;</span><span class="dot two">&#8226;</span><span class="dot three">&#8226;</span>');
        setTimeout(function() {

            $.ajax({
                url: "/item/add-to-wishlist",
                method: 'POST',
                data: $('#add_item_to_wishlist').serialize(),
                success: function(response) {
                    $('#add_to_wishlist_btn').html('Add to Wishlist');
                    $('.wishlist-box h5').html(response.title);
                    $('.wishlist-box p').html(response.msg);
                    $('.popup-wishlist').modal('show');
                }
            });
        }, 2000);
    });
    ...
    $("#price-btn").on('click', function() {
        sessionStorage.setItem('range', $('#amount').val());
    });
    ...
    if (range) {
        ...
        $("#slider-range").slider({
            ...
            slide: function(event, range) {
                $("#amount").val(range.values[0] + "-" + range.values[1]);
            }
        });
    ...
    $(".brief-description").html(function(i, html) {
    ...
    $(window).on('load', function() {
        /*-- preloader ---*/
        $('.preloader-wrap').delay('500').fadeOut(); //change delay back to 1500
        setTimeout(function() {
